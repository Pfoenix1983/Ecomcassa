...
use Bitrix\Sale\Cashbox\CashboxAtolFarmV4;
...
class AtolOnline extends Cashbox implements ICheckable, IPrintImmediately
{
    const SERVICE_URL = "https://app.ecomkassa.ru/fiscalorder/v4" ;
    const TOKEN_OPTION_NAME = 'ecomkassa_access_token';
    ...
    const ERROR_LOGS_DIR = '/ecomkassa_atol_online';
        ...
        $settings = array(
            'AUTH' => array(
                ...
                'ITEMS' => array(
                    'LOGIN' => array(
                        'TYPE' => 'STRING',
                        'LABEL' => Localization\Loc::getMessage('SALE_CASHBOX_ECOMKASSA_SETTINGS_AUTH_LOGIN_LABEL')
                    ),
                    'PASS' => array(
                        'TYPE' => 'STRING',
                        'LABEL' => Localization\Loc::getMessage('SALE_CASHBOX_ECOMKASSA_SETTINGS_AUTH_PASS_LABEL')
                    ),
                )
            ),
            'SERVICE' => array(
    ...
    {
        return Localization\Loc::getMessage('SALE_ECOMKASSA_TITLE');
    }
    ...
    public function check(Check $check)
    {
        ...
        if (empty($EXTERNAL_UUID)) {
            ...
            $r = new Result();
            $r->addError(new Main\Error(Localization\Loc::getMessage('SALE_CASHBOX_ECOMKASSA_CHECK_STATUS_WRONG_UUID')));
            return new $r;
        }
    ...
    private function send($method, $url, array $data = array())
    {
        ...
        if ($response !== false)
        {
            ...
            {
                $this->sendToErrorLog('^^^ Не удалось разобрать ответ от ecomkassa');
        ...
        {
            $this->sendToErrorLog('^^^ Отсутствует ответ от ecomkassa');
            $this->sendToErrorLog('Объект http', $http);
            $this->sendToErrorLog('Объект $response', $response);
            $this->sendToErrorLog('Метод отправки запроса', $method);
            $this->sendToErrorLog('Адресс запроса', $url);
